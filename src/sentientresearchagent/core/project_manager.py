...
class ProjectManager:
    ...
    def __init__(self, projects_dir: Optional[str] = None):
        ...
        self.execution_contexts: Dict[str, 'ProjectExecutionContext'] = {}  # FIXED: Store contexts per project
        self._load_projects()
    ...
    def create_project(self, goal: str, max_steps: int = 250) -> ProjectMetadata:
        ...
        with self._lock:
            ...
            self._save_projects()
            # FIXED: Create execution context on project creation
            self._create_execution_context(project_id)
    ...
    def delete_project(self, project_id: str) -> bool:
        ...
        with self._lock:
            ...
            # FIXED: Cleanup execution context
            if project_id in self.execution_contexts:
                self.execution_contexts[project_id].cleanup()
                del self.execution_contexts[project_id]
    ...
    def _create_execution_context(self, project_id: str) -> 'ProjectExecutionContext':
        """Create and store execution context for project (private)."""
        from ..config import SentientConfig  # Adjust if needed
        from ..hierarchical_agent_framework.agents.registry import AgentRegistry
        from ..hierarchical_agent_framework.agent_blueprints import AgentBlueprint
        
        # Get default config/registry (inject via DI if available)
        config = SentientConfig()  # Or from app context
        registry = AgentRegistry()  # Or from system_manager
        blueprint = None  # Default
        ...
        self.execution_contexts[project_id] = context
        return context

    def get_project_execution_context(self, project_id: str) -> Optional['ProjectExecutionContext']:
        """Get or create execution context for project."""
        if project_id not in self.execution_contexts:
            if project_id in self.projects:
                self._create_execution_context(project_id)
            else:
                logger.warning(f"No project {project_id} found for context")
                return None
        return self.execution_contexts.get(project_id)
