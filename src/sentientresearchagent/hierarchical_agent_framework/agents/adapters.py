...
class PlannerAdapter(LlmApiAdapter):
    ...
    async def process(self, node: TaskNode, agent_task_input: PlannerInput, trace_manager: "TraceManager") -> PlanOutput:
        ...
        if isinstance(result, PlanOutput):
            ...
            parsed_result = await self._extract_and_parse_json(result, PlanOutput)  # FIXED: Added 'await'
            if parsed_result:
            ...
            empty_plan = PlanOutput(sub_tasks=[])  # FIXED: Changed 'sub_goals' to 'sub_tasks'
...
class ExecutorAdapter(LlmApiAdapter):
    ...
    async def process(self, node: TaskNode, agent_task_input: AgentTaskInput, trace_manager: "TraceManager") -> Any:
        ...
        if isinstance(result, str):
            logger.debug(f"    ExecutorAdapter: Wrapping raw string output ({len(result)} chars) in standardized dictionary for node {node.task_id}")  # POLISH: Added better logging
            return {
...
class PlanModifierAdapter(LlmApiAdapter):
    ...
    async def process(self, node: TaskNode, agent_task_input: PlanModifierInput, trace_manager: "TraceManager") -> PlanOutput:
        ...
        if isinstance(result, PlanOutput):
            ...
            # FIXED: Fallback instead of raising
            empty_plan = PlanOutput(sub_tasks=[])  # Use sub_tasks
            logger.warning(f"PlanModifierAdapter: Returning empty plan for node {node.task_id}")
            return empty_plan