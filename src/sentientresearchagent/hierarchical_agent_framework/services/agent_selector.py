...
class AgentSelector:
    ...
    async def select_agent(
        ...
        registry: Optional[AgentRegistry] = None  # NEW: Inject from handler context
    ) -> Optional[str]:
        """
        ...
        Args:
            ...
            registry: Injected registry for named lookups (from handler context)
        """
        # PRIORITY OVERRIDE: Respect node.agent_name FIRST (now with injected registry)
        if node.agent_name and registry:
            adapter = registry.get_named_agent(node.agent_name)
            if adapter:
                logger.info(f"🔧 Selector: Honoring forced agent '{node.agent_name}' for {node.task_id} (via injected registry)")
                return node.agent_name  # Return name; registry handles instance in get_agent_adapter
            else:
                logger.warning(f"🔴 Selector: Named agent '{node.agent_name}' not in injected registry for {node.task_id}")
        
        # NEW: Debug goal warning if override ignored
        if 'debug' in node.goal.lower() and node.agent_name:
            logger.warning(f"🔴 Selector: Debug override '{node.agent_name}' ignored - fell to default for {action_verb}/{task_type or node.task_type}")
