...
from sentientresearchagent.hierarchical_agent_framework.types import TaskType  # Import for enum usage
...
class RegistryIntegrator:
    ...
    def load_and_register_agents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Bootstrap custom agents (e.g., DebuggerAgent) not in yaml
            custom_results = {
            ...
            model_id = config.get('llm', {}).get('model', default_model)
            logger.debug(f"Model ID used: {model_id}")

            custom_registrations = []  # No custom agents needed—DebuggerAgent is YAML now

            # Safeguard: Skip loop if empty, set default results
            if not custom_registrations:
                logger.info("Skipping custom registrations—using YAML agents only.")
            else:
                for custom in custom_registrations:
                    try:
                        # Dynamic import with safety
                        module_path, class_name = custom.get('class_path', '').rsplit('.', 1)
                        if not module_path or not class_name:
                            raise ValueError("Invalid class_path in custom registration.")
                        try:
                            module = __import__(module_path, fromlist=[class_name])
                        except ImportError as ie:
                            raise ImportError(f"Failed to import {module_path}: {ie}. Check sys.path or package install.")
                        agent_class = getattr(module, class_name)
                        
                        # Instantiate with safe kwargs
                        kwargs = {
                            'model_id': custom.get('model_id', model_id),
                            # Add other optional kwargs if needed in future
                        }
                        adapter = agent_class(**kwargs)
                        
                        task_type_enum = TaskType[custom.get('task_type', 'THINK')]
                        
                        # Register
                        self.agent_registry.register_agent_adapter(adapter, action_verb=custom.get('action_verb', 'execute'), task_type=task_type_enum)
                        for name in custom.get('named_keys', []):
                            self.agent_registry.register_agent_adapter(adapter, name=name)
                        
                        # Force THINK for debug goals (bypass planner decomp on root)
                        # Note: agent_task_input/node not in reg—do in dispatch (apply_blueprint_to_node), but for reg, bind stronger
                        logger.info(f"🔧 Registered custom '{custom.get('name', 'unknown')}' (keys: {custom.get('action_verb', 'execute')}/{custom.get('task_type', 'THINK')}, names: {custom.get('named_keys', [])})")
                        custom_results["registered_action_keys"] += 1
                        custom_results["registered_named_keys"] += len(custom.get('named_keys', []))
                        
                        custom_results["details"].append({
                            "name": custom.get('name', 'unknown'),
                            "type": "custom",
                            "action_keys_registered": [(custom.get('action_verb', 'execute'), task_type_enum)],
                            "named_keys_registered": custom.get('named_keys', []),
                            "errors": []
                        })
                        
                    except Exception as e:
                        c_name = custom.get('name', 'unknown')
                        logger.error(f"Failed custom reg for {c_name}: {e}", exc_info=True)
                        custom_results["failed_registrations"] += 1
                        custom_results["details"].append({
                            "name": c_name,
                            "type": "custom",
            ...
            yaml_results = self._register_agents_in_instance_registry()
            
            # Combine results
            combined_results = {
                "registered_action_keys": custom_results["registered_action_keys"] + yaml_results["registered_action_keys"],
                "registered_named_keys": custom_results["registered_named_keys"] + yaml_results["registered_named_keys"],
                "skipped_agents": custom_results["skipped_agents"] + yaml_results["skipped_agents"],
                "failed_registrations": custom_results["failed_registrations"] + yaml_results["failed_registrations"],
                "details": custom_results["details"] + yaml_results["details"]
            }
    ...
    def validate_integration(self) -> Dict[str, Any]:
        ...
        for agent_name, agent_info in self.created_agents.items():
            if "adapter" not in agent_info or "registration" not in agent_info:
                validation_results["issues"].append(f"Missing adapter or registration for {agent_name}")
            ...
            # Check action key registrations (safe get)
            action_keys = registration.get("action_keys", [])
            for action_verb, task_type in action_keys:
                # Handle potential enum/str mismatch
                task_type_for_node = task_type.value if hasattr(task_type, 'value') else task_type
                key = (action_verb.lower(), task_type)
                registered_adapter = self.agent_registry.get_agent_adapter(
                    TaskNode(task_id="validation", goal="validation", task_type=task_type_for_node),
                    action_verb
                )
            ...
            # Check named registrations (safe get)
            named_keys = registration.get("named_keys", [])
    ...
    def load_and_register_profile(self, profile_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Load raw config to get agents section (safe import)
            try:
                from omegaconf import OmegaConf
            except ImportError:
                logger.warning("omegaconf not available—skipping profile load.")
                return {"registered_action_keys": 0, "registered_named_keys": 0, "skipped_agents": 0, "failed_registrations": 0, "details": []}
            ...
            logger.error(f"❌ Failed to integrate profile {profile_name}: {e}", exc_info=True)
            raise
...
def integrate_profile(profile_name: str) -> Dict[str, Any]:
    ...
    Args:
        profile_name: The name of the profile to integrate
...
def apply_blueprint_to_node(node: TaskNode, blueprint: AgentBlueprint, action_verb: str) -> bool:
    ...
    try:
        ...
        if agent_name:
            if 'debug' in node.goal.lower() or (node.layer == 0 and 'debug' in node.goal.lower()):  # Layer 0 = root
                node.task_type = TaskType.THINK
                node.agent_name = 'DebuggerAgent'
                logger.info(f"🔍 Forced DebuggerAgent for debug goal on {node.task_id} (layer {node.layer})")
            else:
                node.agent_name = agent_name
            logger.info(f"✅ Applied blueprint: Node {node.task_id} -> Agent '{node.agent_name}' for action '{action_verb}'")
            return True
        else:
        ...
        logger.error(f"Error applying blueprint to node {node.task_id}: {e}", exc_info=True)
        return False
