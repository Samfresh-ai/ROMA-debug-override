...
class RealtimeExecutionWrapper:
    ...
    async def _periodic_updates(self, execution_task: asyncio.Task):
        ...
        update_interval = 1.5  # Update every 1.5 seconds instead of 2 seconds
        last_update_time = 0
...
class ExecutionService:
    ...
    def __init__(self, project_service, system_manager, socketio):
        """
        ...
        self.socketio = socketio  # Store for threads
        self._running_executions: Dict[str, Dict[str, Any]] = {}
    
    def start_project_execution(self, project_id: str, goal: str, max_steps: int = 250) -> bool:
        """
    ...
    def stop_project_execution(self, project_id: str) -> bool:
        if project_id in self._running_executions:
            # Signal cancellation (e.g., via shared flag or engine.stop())
            logger.info(f"🛑 Stopping execution for {project_id}")
            del self._running_executions[project_id]
            return True
        return False
    ...
    async def _run_project_cycle_async(self, project_id: str, goal: str, max_steps: int):
        ...
        try:
            ...
            start_time = time.time()  # Hoist here, before if/else
            ...
            if project_state and 'all_nodes' in project_state and len(project_state['all_nodes']) > 0:
                ...
                total_time = time.time() - start_time  # Add this after result = await ...
                ...
                if isinstance(result, dict) and 'error' in result:
                    ...
                    try:
                        if hasattr(error_msg, 'model_dump'):
                            error_msg = error_msg.model_dump()
                        elif isinstance(error_msg, BaseModel):
                            error_msg = error_msg.dict()
                    except Exception as dump_err:
                        logger.warning(f"Failed to serialize error: {dump_err}")
                        error_msg = str(error_msg)
                    self.socketio.server.emit('project_complete', {
                        'project_id': project_id,
                        'status': 'FAILED',
                        'error': error_msg,
                        'timestamp': datetime.now().isoformat()
                    }, namespace='/')
                    logger.info(f"📡 Emitted 'project_complete' (FAILED) for {project_id}")
                    ...
                    root_node = project_task_graph.get_node('root') if project_task_graph else None
                    root_result = root_node.result if root_node else 'Execution completed - no root result available'
                    try:
                        # Dump Pydantic models to dict for JSON
                        if hasattr(root_result, 'model_dump'):
                            root_result = root_result.model_dump()  # e.g., AggregatorOutput to dict
                        elif isinstance(root_result, BaseModel):
                            root_result = root_result.dict()  # Fallback for older Pydantic
                    except Exception as dump_err:
                        logger.warning(f"Failed to serialize result: {dump_err}")
                        root_result = str(root_result)  # String fallback
                    self.socketio.server.emit('project_complete', {
                        'project_id': project_id,
                        'status': 'DONE',
                        'root_result': root_result,
                        'total_nodes': len(project_task_graph.nodes) if project_task_graph else 0,
                        'execution_time': total_time,  # Use your total_time var
                        'timestamp': datetime.now().isoformat()
                    }, namespace='/')
                    logger.info(f"📡 Emitted 'project_complete' for {project_id}")

                    # FIXED: Emit full state update after complete
                    context = self.project_service.get_project_execution_context(project_id)
                    if context:
                        full_state = context.save_state()
                        self.socketio.server.emit('project_state_updated', {
                            'projectId': project_id,
                            'data': full_state
                        }, namespace='/')
                        logger.info(f"📡 Emitted full state update after complete for {project_id}")
            else:
    ...
    async def _run_configured_project_cycle_async(self, project_id: str, goal: str, max_steps: int, config: SentientConfig):
        ...
        try:
            await self._run_project_cycle_async(project_id, goal, max_steps)  # Call shared logic
            total_time = time.time() - start_time  # Use this in emit
            
            # Get root_node for emit (fix the attribute error)
            root_node = None
            project_components = self.project_service.get_or_create_project_graph(project_id)  # Use project_service
            if project_components.get('task_graph'):
                project_task_graph = project_components['task_graph']
                root_node = project_task_graph.get_node('root')
            
            root_result = root_node.result if root_node else 'Configured execution completed - no root result available'
            try:
                # Dump Pydantic models to dict for JSON
                if hasattr(root_result, 'model_dump'):
                    root_result = root_result.model_dump()  # e.g., AggregatorOutput to dict
                ...
                logger.warning(f"Failed to serialize result: {dump_err}")
                root_result = str(root_result)  # String fallback
            # Emit (now using self.socketio)
            self.socketio.server.emit('project_complete', {
                'project_id': project_id,
                'status': 'DONE',
                'root_result': root_result,
                'total_nodes': len(project_task_graph.nodes) if 'task_graph' in project_components else 0,
                'execution_time': total_time,
                'timestamp': datetime.now().isoformat()
            }, namespace='/')
            logger.info(f"📡 Emitted 'project_complete' (configured) for {project_id}")

            # FIXED: Same for configured
            context = self.project_service.get_project_execution_context(project_id)
            if context:
                full_state = context.save_state()
                self.socketio.server.emit('project_state_updated', {
                ...
                logger.info(f"📡 Emitted full state update after configured complete for {project_id}")
            
        except Exception as e:
            logger.error(f"Configured execution failed for {project_id}: {e}")
            ...
            try:
                ...
                logger.warning(f"Failed to serialize error: {dump_err}")
                error_msg = str(error_msg)
            # Emit error (using self.socketio)
            self.socketio.server.emit('project_complete', {
                'project_id': project_id,
                'status': 'FAILED',
                'error': error_msg,
            ...
            logger.info(f"📡 Emitted 'project_complete' (FAILED, configured) for {project_id}")
    ...
    def _save_final_project_state_enhanced(self, project_task_graph, project_id):
        ...
        try:
            ...
            if successful_saves > 0:
                ...
                # FIXED: Emit deserialized full state
                context = self.project_service.get_project_execution_context(project_id)
                if context:
                    full_state = context.save_state()  # Serialized already
                    self.socketio.server.emit('project_state_updated', {
                        'projectId': project_id,
                        'data': full_state
                    }, namespace='/')
                    logger.info(f"📡 Emitted full state update for {project_id}")
            else:
