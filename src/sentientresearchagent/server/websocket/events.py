...
def register_websocket_events(socketio, project_service, execution_service):
    ...
    def handle_connect(auth):
        ...
        try:
            # FIXED: Send initial state + check for complete projects on reconnect
            handle_request_initial_state()
            # NEW: Emit full project state if active project is DONE (syncs loading UI)
            current_project = project_service.project_manager.get_current_project()
            if current_project and current_project.status == 'DONE':
                state = project_service.get_project_display_data(current_project.id)
                emit('project_state', {
                    'status': 'DONE',
                    'rootResult': state.get('root_result', 'No result available'),
                    'projectId': current_project.id
                })
                logger.info(f"🔄 Reconnect: Emitted DONE state for project {current_project.id}")
    ...
    def handle_request_initial_state():
        ...
        try:
            ...
            if current_project:
                ...
                # FIXED: Always emit current state on initial load
                state = project_service.get_project_display_data(current_project.id)
    ...
    def handle_check_status(data):
        """NEW: Poll handler for status (fallback for reconnects/loading)."""
        logger.debug(f"🔍 Status poll for project: {data.get('projectId')}")
        try:
            project_id = data.get('projectId')
            if not project_id:
                emit('project_state', {'status': 'ERROR', 'message': 'No projectId'})
                return
            
            state = project_service.get_project_display_data(project_id)
            if state and state.get('status') == 'DONE':
                emit('project_state', {
                    'status': 'DONE',
                    'rootResult': state.get('root_result', 'No result'),
                    'projectId': project_id
                })
                logger.debug(f"🔍 Poll: Emitted DONE for {project_id}")
            else:
                emit('project_state', state or {'status': 'NOT_FOUND'})
        except Exception as e:
            logger.error(f"Status poll error: {e}")
            emit('project_state', {'status': 'ERROR', 'message': str(e)})
