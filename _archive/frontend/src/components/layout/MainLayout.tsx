...
import FullResultModal from '@/components/panels/FullResultModal'
...
const MainLayout: React.FC = () => {
  ...
  const mockNode = { task_id: 'project-root', goal: 'Final Report', status: 'DONE', full_result: { output_text: '' } }
  ...
  return (
    <div className="h-screen flex bg-background">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <FullResultModal 
          isOpen={showFinalReportModal}  // NEW: Use store switch
          onClose={() => useTaskGraphStore.getState().setShowFinalReportModal(false)}  // NEW: Flip off on close
          node={mockNode || { task_id: 'project-root', goal: 'Final Report', status: 'DONE', full_result: { output_text: '' } }}  // NEW: Mock node for report (or pass real root)
        />
      </div>
    </div>
  )
