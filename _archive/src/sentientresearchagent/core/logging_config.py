...
def setup_logging(config: 'LoggingConfig', console_filter: Optional[callable] = None):
    ...
    # FIXED: Add custom log levels safely (check existence first)
    custom_levels = [
        ("PLAN", 25, "<blue>"),
        ("EXECUTE", 26, "<magenta>")
    ]
    ...
    for name, no, color in custom_levels:
        if name not in logger._core.levels:
            logger.level(name, no=no, color=color)
            logger.info(f"Added custom level: {name}")
        else:
            logger.debug(f"Custom level {name} already exists - skipping")
