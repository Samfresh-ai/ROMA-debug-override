...
# FIXED: Load config at top (after imports, if not there)
from sentientresearchagent.config import load_config
config = load_config()  # Loads yaml/.env globally
...
class CycleManager:
    ...
    async def execute_step(
        ...
        node_processor: NodeProcessorType,  # FIXED: No config param—use global
        knowledge_store: KnowledgeStore,
        update_callback: Optional[Callable] = None
    ) -> bool:
        """
        ...
        while iteration < max_iterations:
            ...
            for node in plan_done_nodes:
                ...
                # Also check if this was originally an EXECUTE node that somehow got planned
                    ...
                    # FIXED: Soft unblock on timeout - mark NEEDS_REPLAN for retry (no force-fail, logs for ROMA)
                    planned_time = node.timestamp_planned or time.time()
                    loop_timeout = config.execution.loop_timeout_seconds  # FIXED: From yaml
                    if time.time() - planned_time > loop_timeout:
                        # FIXED: Rich log for ROMA (timestamp, children stats, input for replay)
                        total_children = len(node.children or [])
                        done_children = sum(1 for child in (node.children or []) if child.status == TaskStatus.DONE)
                        logger.warning(f"🚨 Soft unblock {node.task_id} after {loop_timeout}s: {done_children}/{total_children} children DONE. "
                                      f"Input size: {len(str(node.aux_data.get('final_llm_input', '')))} chars. Replan for retry.")
                        node.aux_data['replan_reason'] = f"Stuck PLAN_DONE >{loop_timeout}s"  # FIXED: Metadata for ROMA trace
                        if await _atomic_transition_if_eligible(node, TaskStatus.PLAN_DONE, TaskStatus.NEEDS_REPLAN,
                                                               lambda: True,  # Force eligible for unblock
                                                               knowledge_store, update_callback):
                            made_plan_done_transition = True
                            any_transition_this_iteration = True
                            processed_in_step = True
                            continue  # Skip normal check, re-eval next cycle
                    else:
                        logger.debug(f"Node {node.task_id} cannot AGGREGATE: Not all sub-tasks in '{node.sub_graph_id}' are finished.")
                        # ... your existing blocking log
