...
from sentientresearchagent.hierarchical_agent_framework.context.agent_io_models import (
    ...
    CustomSearcherOutput, PlanModifierInput, ExecutorOutput, AggregatorOutput
)
...
from sentientresearchagent.hierarchical_agent_framework.toolkits.data import CoinGeckoToolkit, ArkhamToolkit, DefiLlamaToolkit
from .models import (
    ...
    def create_tools(self, tool_configs: List[Union[str, Dict[str, Any]]]) -> List[Any]:
        ...
        for config in tool_configs:
            ...
            if tool_name == "web_search":
                try:
                    ...
                    logger.warning(f"Failed to import web_search: {e}. Skipping web_search tool.")
                break
        ...
        for config in tool_configs:
            if isinstance(config, str):
                ...
                logger.warning(f"Invalid tool configuration type: {type(config)}. Skipping.")
                continue
            ...
            if tool_name in self._tools:
                try:
                    ...
                    logger.warning(f"Failed to create tool {tool_name} with params: {tool_params} - {e}. Skipping.")
            elif tool_name == "web_search" and web_search is not None:
                tools.append(web_search)
                logger.debug("Added web_search function as tool")
            else:
                logger.warning(f"Tool {tool_name} not found in _tools. Available tools: {list(self._tools.keys())}. Skipping.")
