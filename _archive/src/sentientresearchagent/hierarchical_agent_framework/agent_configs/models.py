...
# Suppress Pydantic namespace warnings module-wide (fires during field scan)
warnings.filterwarnings("ignore", message=".*protected namespace.*", category=UserWarning, module="pydantic._internal._fields")
...
__all__ = [
    ...
    "ProfileConfig", "ProfileYAMLConfig", "ExecutorOutput", "AggregatorOutput",
...
class ModelConfig(BaseModel):
    ...
    model_config = ConfigDict(
        protected_namespaces=(),  # Disables all protected namespace checks
        extra='allow'
    )
...
class ToolkitConfig(BaseModel):
    ...
    def validate_toolkit_config(self):
        ...
        # Log warning for unknown toolkits
        else:
            logger.warning(f"Unknown toolkit '{name}' - skipping parameter validation.")
...
class AgentConfig(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        extra='allow',  # Allows model_override
        validate_assignment=True  # Applies config during field assignment
    )
    ...
    # Model configuration
    model_override: Optional[str] = Field(None, description="Override for model config (e.g., dynamic model_id)")
    ...
    def validate_agent_config(self):
        ...
        if self.response_model:
            response_model_requirements = {
                ...
                "executor": ["WebSearchResultsOutput", "CustomSearcherOutput", "ExecutorOutput", None],  # Added ExecutorOutput
                "aggregator": ["AggregatorOutput", None],  # Added AggregatorOutput
