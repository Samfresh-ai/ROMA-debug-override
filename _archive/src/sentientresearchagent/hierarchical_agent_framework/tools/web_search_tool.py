...
print("Starting to load web_search_tool.py - before imports")
...
print("Finished standard imports in web_search_tool.py")
...
def web_search(query: str) -> str:
    ...
    print("Inside web_search function - starting execution")
    ...
    try:
        ...
        if gemini_key:
            print(f"Using Gemini with key (masked): {gemini_key[:5]}...")
...
Answer:"""
            ...
            print(f"Sending request to Gemini URL: {url}")
            ...
            if response.status_code == 200:
                ...
                print(f"Gemini response failed with status: {response.status_code}")
        
        print("No Gemini key found, checking OpenAI")
        # Fallback to OpenAI if available
        openai_key = os.getenv("OPENAI_API_KEY")
        if openai_key:
            print(f"Using OpenAI with key (masked): {openai_key[:5]}...")
            ...
            print(f"Sending request to OpenAI URL: {url}")
            ...
            if response.status_code == 200:
                ...
                print(f"OpenAI response failed with status: {response.status_code}")
        
        print("No API keys found for web search")
        ...
        print(f"Exception in web_search: {error_msg}")
        import traceback
...
def clean_tools(tools: List[Any]) -> List[Any]:
    ...
    """
    print("Starting clean_tools function")
    ...
    print(f"Cleaned {len(tools)} tools")
    return cleaned
...
print("Finished loading web_search_tool.py")