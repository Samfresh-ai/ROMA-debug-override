...
class ExecuteHandler(BaseNodeHandler):
    ...
    async def _get_agent_for_node(
        ...
        """
        # NEW: Short-circuit for forced named agents (e.g., debug overrides)
        if node.agent_name:
            adapter = context.agent_registry.get_named_agent(node.agent_name)
            if adapter:
                logger.info(f"🔧 ExecuteHandler: Honoring forced agent '{node.agent_name}' for {node.task_id} (skipping selector)")
                return adapter
            else:
                logger.warning(f"🔴 Named agent '{node.agent_name}' not in registry for {node.task_id}")
        
        # Use agent selector service (existing logic)
