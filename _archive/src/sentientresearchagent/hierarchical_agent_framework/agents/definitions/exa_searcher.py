...
class ExaCustomSearchAdapter(BaseAdapter):
    ...
    model_id: str = "groq/llama-3.3-70b-versatile"  # Default model for LiteLLM, can be configured

    def __init__(self, exa_client=None, model_id: str = "groq/llama-3.3-70b-versatile", num_results: int = 5, **kwargs):
        ...
        self.num_results = 3  # NEW: Fewer results = less text
    ...
    async def process(self, node: TaskNode, agent_task_input: AgentTaskInput, trace_manager: "TraceManager") -> Dict:
        ...
        try:
            ...
            exa_results = await asyncio.to_thread(
                ...
                use_autoprompt=True,  # Auto-formats, replaces 'context'
            )
            ...
            #NEW: Truncate long sources to avoid context overflow
            formatted_sources = [source_content[:2000] + "\n[TRUNCATED - Full content too long]" if len(source_content) > 2000 else source_content for source_content in formatted_sources]
...
IMPORTANT: Include EVERY piece of relevant data from ALL sources. Do not summarize or truncate."""
            ...
            llm_response = await acompletion(
                ...
                max_tokens=2000,  # NEW: Cap output to 2000 tokens (prevents rambling)
                # truncate_prompt=True,  # NEW: Auto-cut input if too long (if LiteLLM supports)
            )
