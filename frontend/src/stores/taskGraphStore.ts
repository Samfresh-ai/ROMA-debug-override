...
export const useTaskGraphStore = create<TaskGraphState>()(
  subscribeWithSelector((set, get) => ({
    ...
    setData: (data: APIResponse) => {
      ...
      // FIXED: Deserialize raw API data (str enums/datetimes → objects)
      ...
      const newNodes = deserializedData.all_nodes || {}
      ...
      const newState = {
        ...
        graphs: { ...(deserializedData.graphs || {}) },
        overallProjectGoal: deserializedData.overall_project_goal,
      ...
      // FIXED: Set root result for modal (if complete)
      if (deserializedData.all_nodes && Object.keys(deserializedData.all_nodes).length > 0) {
        const rootNode = Object.values(deserializedData.all_nodes)[0] as TaskNode;  // Assume first is root
        if (rootNode.full_result) {
          set({
            finalReport: rootNode.full_result.output_text || JSON.stringify(rootNode.full_result),
            showFinalReportModal: true  // Auto-open on load
          });
        }
      }
    ...
    setShowFinalReportModal: (open: boolean) => {
      set({ showFinalReportModal: open })
    },
    ...
    setProjectData: (projectId: string, data: APIResponse) => {
      console.log('🏪 STORE: Set project data:', {
        projectId,
        nodeCount: Object.keys(data.all_nodes || {}).length,
        goal: data.overall_project_goal?.substring(0, 50)
      });
      
      // FIXED: Deserialize raw data
      const deserializedData = deserializeState(data);
      const projectData: ProjectData = {
        nodes: deserializedData.all_nodes || {},
        graphs: deserializedData.graphs || {},
        overallProjectGoal: deserializedData.overall_project_goal,
    ...
    switchToProject: (projectId: string) => {
      ...
      if (projectData) {
        // FIXED: Deserialize for display
        const apiResponse: APIResponse = deserializeState({
...
if (typeof window !== 'undefined' && window) {
  ...
  useTaskGraphStore.subscribe((state) => {
    if (state.currentProjectId) {
      console.log('🏪 STORE: Current project switched to:', state.currentProjectId, 'with', Object.keys(state.nodes).length, 'nodes');
    }
  });
