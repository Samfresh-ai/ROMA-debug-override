...
import { deserializeState, deserializeNodeData } from '@/utils/serialization';
...
class WebSocketService {
  ...
  private setupEventListeners() {
    ...
    this.socket.on('project_state_updated', (rawData: {projectId: string, data: any}) => {
  const currentProjectId = useProjectStore.getState().currentProjectId;
  if (rawData.projectId === currentProjectId) {
    console.log('🆙 Raw state received:', rawData.data.all_nodes?.length || 0, 'nodes');
    const deserializedData = deserializeState(rawData.data);  // FIXED: Deserialize
    console.log('🆙 Deserialized state:', deserializedData.all_nodes?.length || 0, 'nodes');
    const taskGraphStore = useTaskGraphStore.getState();
    taskGraphStore.setData(deserializedData);  // Use deserialized
    taskGraphStore.setLoading(false);
  }
});
    
    this.socket.on('project_complete', (msg) => {
      const currentProjectId = useProjectStore.getState().currentProjectId;
      if (msg.project_id === currentProjectId) {
        console.log('✅ Complete, nodes:', msg.total_nodes);
        const taskGraphStore = useTaskGraphStore.getState();
        ...
        // Existing logic
        console.log('🎉 Project complete received:', msg);
        
        // Force update store with final data
        if (msg.project_id) {
          // FIXED: Deserialize before setProjectData
          const deserializedMsg = deserializeState(msg);
          taskGraphStore.setProjectData(msg.project_id, deserializedMsg);
          if (msg.project_id === taskGraphStore.currentProjectId) {
            taskGraphStore.setData(deserializedMsg);
          }
        }
        
        // Show report in UI (adapt to your component, e.g., set report state)
        if (msg.root_result) {
          console.log('📄 Final report:', msg.root_result);
          useTaskGraphStore.getState().setFinalReport(msg.root_result);  // NEW: Use your new action
          useTaskGraphStore.getState().setShowFinalReportModal(true);
          window.dispatchEvent(new CustomEvent('showFinalReport', { detail: { report: msg.root_result } }));
        }
        
        // Optional: Auto-save final state
        if (msg.project_id) {
          this.autoSaveProjectData(msg.project_id, msg);
        }
      }
    });
  }
  ...
  private autoSaveProjectData(projectId: string, data: any) {
    try {
      ...
      const serializedData = JSON.stringify(saveData);  // Already str
      // FIXED: No need for TS serialize—backend handles on load
      localStorage.setItem(`project_${projectId}_backup`, serializedData);
